...
export default function TaskDetailsScreen({navigation, route}) {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', loadTask);
  ...
  return (
    <View style={styles.background}>
      <ScrollView
        ...
        {loaded && (
          <View style={styles.contentContainer}>
            <Text onPress={loadTask}>
              <Tag header="Von" content={task.from} />
              <Tag
                header="Fertig"
                content={getTagContent(task.done.toString())}
              />
              <Tag
                header="Feedback"
                content={getTagContent(task.feedback.toString())}
              />
              <Tag
                header="Start"
                content={
                        ...
                        onPress={() => {
                          ...
                          iserv.init().then(() => {
                            if (file.url && file.name) {
                              ...
                              Alert.alert(
                                'Download gestartet',
                                'Schau in deine Benachrichtigungen',
                              );
                            }
                          });
                        }}
        ...
        {!loaded && error !== '' && <ListError icon={'bug'} error={error} />}
      </ScrollView>
    </View>
  );
}
